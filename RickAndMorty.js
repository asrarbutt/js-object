...
const getAliveCharacters = () => {
    return rickAndMortyAPI.results.filter(e => e.status === "Alive");
};
const getAllCharactersName = () => {
    return rickAndMortyAPI.results.map(e => e.name);
};
let newObj = rickAndMortyAPI.results.map(emp => {
    return { name: emp.name, origin: emp.origin };
});
console.log(newObj);
